...
from get_gainer.selenium_chrome_controller import SeleniumChromeController
controller = SeleniumChromeController()
html = controller.get_html("https://finance.yahoo.com/research-hub/screener/most_active_penny_stocks/?start=0&count=50", use_debugging=False, headless=False)
...
def get_realtime_penny_gainers(price_max=1.0, count=10):
    ...
    try:
        ...
        # 格式化結果，去掉Name列並將% Change顯示為更細的數字
        gainers['Price'] = gainers['Price'].round(2)
        gainers['% Change'] = gainers['% Change'].round(4)  # 將%變化顯示為4位小數
        
        if not gainers.empty:
            # 只保留需要的列，去掉Name
            gainers = gainers[['Symbol', 'Price', '% Change']]
...
if __name__ == "__main__":
    try:
        ...
        print(penny_stocks[['Symbol', 'Price', '% Change']].to_string(index=False))  # 去掉Name列並顯示% Change
